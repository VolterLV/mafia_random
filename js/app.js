 ...
 */
window.onload = function () {
	/* 	const info = document.querySelector('.content-main__title')
	const input = document.querySelector('.input')
	const text = document.querySelector('.content-main__text')
	const subtext = document.querySelector('.content-main__subtext')
	const button1 = document.querySelector('.header__button1')
	const name = 'Volter'
	...
	function setInfo() {
		info.innerHTML =
			...
			` b= ${masA.b}<br>  ` +
			` login= ${masA.login}<br>` +
			`popular= ${masA.popular}<br>`
		masA.c = 30
		text.innerHTML = `${Object.values(masA)}`
		masA['popular'] = true
		masAnew = Object.assign({}, masA)
		masJS = JSON.stringify(masA)
		masAnew.a = 100
		masAnew['b'] = name
		subtext.innerHTML = `${JSON.stringify(masAnew)}`
	}
	function plus(a = masA.a, b = masA.b) {
		let number
		number = a + b
	...
	function auto() {
		...
		if (quantity_user > 30) {
			...
			if (quantity_user == '' || quantity_user == undefined) {
				...
				while (roles_massiv.length != quantity_user) {
				...
				for (
					let counter = 0;
					counter < quantity_user_massiv.length;
					counter = counter + 1
				) {
					...
					if (roles_massiv[rand] == 'Мирный') {
						picture = 'img/cute-new-born-lamb-bob-van-den-berg-photography.jpg'
					}
...
`
				...
				console.log('Длина массива с людьми: ' + quantity_user_massiv.length)
				console.log('Массив с ролями: ' + roles_massiv)
				console.log('Массив с кол-во человек: ' + quantity_user_massiv)
			}
		}
	}
}
